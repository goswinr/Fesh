...
module Commands = 
    ...
    let RunAllText       = "Run All Text"             , "F5"            , mkCmd isTab       (fun a -> save Tab.currTab|>ignore; agent.Post (Evaluate Tab.currEditor.Text)) ,"Send all text in teh current file to FSharp Interactive"
    ...
    // todo refine 
    let ToggleLogSize    = "Maximise Maximised Log Window" , "Ctrl + M" , mkCmdSimple (fun a -> WindowLayout.maxLog()) ,"Maximises or resets the size of the Log window. \r\n(depending on curren state)"
    ...
    let AllInputBindings = 
        ...
        // this is to make sure the displayed gestuer matches the actual gesture
        let getKey = function
            ...
            | x -> match Key.TryParse(x,true) with
                   |true, k -> k
                   | _ -> Log.printf "*failed to parse Command Key '%A'" x; Key.NoName
        
        let getModKey = function
            |"Ctrl"     -> ModifierKeys.Control        
            | x -> match ModifierKeys.TryParse(x,true) with
                   |true, k -> k
                   | _ -> Log.printf "*failed to parse ModifierKey '%A'" x; ModifierKeys.None

        try
            [|
                 for _,g,cmd,tt in allCustomCommands do
                    match g.Trim() with
                    |"" -> ()
                    | "Ctrl + '+'" | "Ctrl + +" | "Ctrl +" ->
                    ...
                    | gg -> 
                        yield
                            ...
                            | _ -> 
                                Log.printf "*failed to parse command Input gesture '%s'" g
                                InputBinding(cmd,  KeyGesture(Key.None))
            |]
        with e -> 
            Log.printf "*failed to create keyboard shortcuts: %A"e
            [| |]
