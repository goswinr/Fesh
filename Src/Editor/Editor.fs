...
type Editor private (code:string, config:Config, filePath:FilePath)  = 
    ...
    static member SetUp  (code:string, config:Config, filePath:FilePath ) = 
        ...
        avaEdit.TextArea.AlternativeRectangularPaste <- Action<string,bool>( fun txt txtIsFromOtherRectSel -> RectangleSelection.insertText(ed,txt)) //TODO check txtIsFromOtherRectSel on pasting text with \r\n
