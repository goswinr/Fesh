...
module DocStringNewUNUSEDYET =
    ...
    module private Section =
        ...
        let inline addSection (name : string) (content : string) =
            if name <> "" then
                nl + nl
                + name + ":"  //--modified for Seff--
                //+ "**" + name + "**"  //original FsAutocomplete
                + nl + nl + content
            else
                nl + nl + content
        ...
        let fromKeyValueList (name : string) (content : list<KeyValuePair<string, string>>) =
            if List.isEmpty content then
                ...
                |> Seq.map (fun kv ->
                    ...
                    //"* `" + kv.Key + "`" + ": " + text //original FsAutocomplete
                    "• `" + kv.Key + "`" + ": " + text //--modified for Seff--
                )
                |> String.concat nl
                |> addSection name
    ...
    module private Format =
        ...
        let private itemListToStringAsMarkdownList (prefix : string) (item : ItemList) =
            ...
            | DescriptionOnly description ->
                prefix + " " + description
            | TermOnly term ->
                //prefix + " " + "**" + term + "**" //original FsAutocomplete
                prefix + " "  + term  //--modified for Seff--
            | Definitions (term, description) ->
                //prefix + " " + "**" + term + "** - " + description  //original FsAutocomplete
                prefix + " " + term + " - " + description//--modified for Seff--
                ...
                Formatter =
                    ...
                    | NonVoidElement (innerText, attributes) ->
                        ...
                        | Bulleted ->
                            ...
                            items
                            //|> List.map (itemListToStringAsMarkdownList "*")//original FsAutocomplete
                            |> List.map (itemListToStringAsMarkdownList "• ") //--modified for Seff--
                            |> String.concat "\n"
    ...
    type (*private*) XmlDocMember(doc: XmlDocument, indentationSize : int, columnOffset : int) = //--modified for Seff--
        ...
        let seeAlso =
            ...
            |> Seq.cast<XmlNode>
            |> Seq.map (fun node ->
                // "* `" + Format.extractMemberText node.Attributes.[0].InnerText + "`" //original FsAutocomplete
                "• `" + Format.extractMemberText node.Attributes.[0].InnerText + "`" //--modified for Seff--
            )
        ...
        member __.ToSummaryOnlyString() =
            ...
            if String.IsNullOrEmpty summary then
                ...
                // "**Description**" + nl + nl + summary //original FsAutocomplete
                summary//--modified for Seff--
        ...
        member __.ToFullEnhancedString() =
            ...
            if String.IsNullOrEmpty content then
                ...
                //"**Description**" + nl + nl  + content //original FsAutocomplete
                content //--modified for Seff--
    ...
    let (* private*) getXmlDoc dllFile = //--modified for Seff--
    ...
    let private buildFormatComment cmt (formatStyle : FormatCommentStyle) (typeDoc: string option) =
        ...
        | FSharpXmlDoc.Text (document) -> // back to FCS 33.0.1
            try
                ...
                *)
                let xmlDoc = XmlDocMember(doc, 0, 0)
            ...
            with
                | ex ->                    //--modified for Seff--
                    sprintf "An error occured when parsing the doc comment, please check that your doc comment is valid: %A" ex
