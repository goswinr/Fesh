...
type Seff (config:Config,log:Log) =
    ...
    do
        ...
        if config.HostingInfo.IsStandalone then win.Window.ContentRendered.Add(fun _ -> log.PrintInfoMsg "* Time for loading and rendering of main window: %s"  Timer.InstanceStartup.tocEx) 
