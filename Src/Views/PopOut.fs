...
module PopOut = 
    ...
    let showWindow(title, getUi:unit->#UIElement, parent:Window) =         
        ...
        if statusBar.CheckerStatus.IsMouseOver || ed.ErrorHighlighter.ToolTip.IsOpen then
            match statusBar.CheckerStatus.ToolTip with
                | :? ToolTip as tt -> 
                    ...
                    | :? StackPanel  -> showWindow ("Seff PopOut| Compiler Error Info ", statusBar.CheckerStatus.GetErrorPanelCached, parent)
                    | _ -> () // if ToolTip is just a string ?? dont pop out                
                | :? StackPanel  ->showWindow ("Seff PopOut| Compiler Error Info ", statusBar.CheckerStatus.GetErrorPanelCached, parent)
                | _ -> () // if ToolTip is just a string?? dont pop out

        if ed.TypeInfoTip.IsOpen then
            showWindow ("Seff PopOut| Type Info ", TypeInfo.getPanelCached, parent)

        if ed.Completions.IsOpen  && ed.Completions.HasStackPanelTypeInfo then
            showWindow ("Seff PopOut| Autocomplete Type Info ", TypeInfo.getPanelCached, parent)
