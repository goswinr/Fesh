...
module PopOut =
    ...
    let icon =
        try
            Windows.Media.Imaging.BitmapFrame.Create(Uri("pack://application:,,,/Fesh;component/Media/logoGray.ico"))
        with ex ->
    ...
    let create(grid:TabsAndLog, statusBar:FeshStatusBar) =
        ...
        if statusBar.CheckerStatus.IsMouseOver || ed.DrawingServices.errors.ToolTip.IsOpen then
            match statusBar.CheckerStatus.ToolTip with
                | :? ToolTip as tt ->
                    ...
                    | :? StackPanel as sp -> showWindow ("Fesh PopOut| Compiler Error Info ", sp.ActualWidth, sp.ActualHeight, (fun () -> statusBar.CheckerStatus.GetErrorPanelCached ed) , parent)
        ...
        if ed.TypeInfoTip.IsOpen then
            ...
            showWindow ("Fesh PopOut | Type Info ", ti.ActualWidth, ti.ActualHeight ,(fun () -> Some newSV), parent)
