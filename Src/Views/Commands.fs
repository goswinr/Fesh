...
type Commands (grid:TabsAndLog, statusBar:FeshStatusBar)  =
    ...
    member val SwapLineUp        = {name= "Swap Lines Up"             ;gesture= "Alt + Up"       ;cmd = mkCmdSimple (fun _ -> SwapLines.swapLinesUp  (curr()))     ;tip="Swaps the current line(s) with the previous line."  }
    member val SwapLineDown      = {name= "Swap Lines Down"           ;gesture= "Alt + Down"     ;cmd = mkCmdSimple (fun _ -> SwapLines.swapLinesDown(curr()))     ;tip="Swaps the current line(s) with the next line."  }
    ...
    member val GoToError         = {name= "Scroll to Errors"              ;gesture= "Ctrl + E"        ;cmd= mkCmdSimple (fun _ -> goToError())             ;tip="Scrolls step by step through error segments. Unfold if needed." }
    ...
    // View menu:
    member val ToggleSplit       = {name= "Toggle Window Split"           ;gesture= ""            ;cmd= mkCmdSimple (fun _ -> grid.ToggleSplit())         ;tip="Toggles between vertical and horizontal window arrangement of Editor and Log View." }
