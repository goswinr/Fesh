...
type Commands (grid:TabsAndLog, statusBar:SeffStatusBar)  = 
    ...
    member val CompileScriptSDK  = {name= "Compile Script via dotnet SDK" ;gesture= "Ctrl + B"        ;cmd= mkCmdSimple (fun _ -> compileScr(false))  ;tip= "Create an net48 fsproj with current code (including unsaved changes) and build it via 'dotnet build' in Release x64. dotnet SDK needs to be installed."}
    member val CompileScriptMSB  = {name= "Compile Script via MSBuild"    ;gesture= "Ctrl + Shift + B";cmd= mkCmdSimple (fun _ -> compileScr(true))   ;tip= "Create an net48 fsproj with current code (including unsaved changes) and build it via  MSBuild in Release x64. VS2019 needs to be installed."}
   ...
   //View menu:
    ...
    //member val ReloadXshdFile    = {name= "Reload SyntaxHighlighting" ;gesture= "F10"            ;cmd= mkCmdSimple (fun _ -> SyntaxHighlighting.setFSharp(tabs.CurrAvaEdit,true))  ;tip= "Reloads FSharpSynatxHighlighterExtended.xshd, this is useful for testing new highlighting files without a restart." }
    member val OpenXshdFile      = {name= "Open and watch SyntaxHighlighting in VsCode" ;gesture= ""  ;cmd= mkCmdSimple (fun _ -> SyntaxHighlighting.openVSCode(tabs.CurrAvaEdit))   ;tip= "Opens the FSharpSynatxHighlighterExtended.xshd, file in VsCode.\r\nWatches the file for changes and reloads automatically." }
